...
////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  intrinsic word propertyIndex(bUseVexI2CEncoderVelocity,			propertySystem, kSystemIncludeVelocityInVexCortexI2C);
#endif
...
intrinsic void deprecated nxtEraseEllipse(const int Left, const int Top, const int Right, const int Bottom)
                asm(opcdDrawOnGraphicsLCDDisplay, byte(displayDrawEraseEllipse),  variable(Left), variable(Top), variable(Right), variable(Bottom));
intrinsic void deprecated drawInvertEllipse(const int Left, const int Top, const int Right, const int Bottom)
                asm(opcdDrawOnGraphicsLCDDisplay, byte(displayDrawInvertEllipse),  variable(Left), variable(Top), variable(Right), variable(Bottom));

intrinsic void deprecated nxtDrawRect(const int Left, const int Top, const int Right, const int Bottom)
                asm(opcdDrawOnGraphicsLCDDisplay, byte(displayDrawRect),  variable(Left), variable(Top), variable(Right), variable(Bottom));
intrinsic void deprecated drawInvertRect(const int Left, const int Top, const int Right, const int Bottom)
                asm(opcdDrawOnGraphicsLCDDisplay, byte(displayDrawInvertRect),  variable(Left), variable(Top), variable(Right), variable(Bottom));
...
intrinsic word property(SensorValue,              propertySensor,         kNumbOfTotalSensors, tSensors);
...
#define motorPowerSigned motor
  ...
  intrinsic void setMotorTarget(tMotor nMotor, long nEncoderCountTarget, int nMaxSpeedToUse, bool bHoldAtEnd = true)
  ...
  intrinsic void moveMotorTarget(tMotor nMotor, long nEncoderCountTarget, int nMaxSpeedToUse, bool bHoldAtEnd = true)
...
#if (bSmartMotorsWithEncoders() != 0) || defined(VexIQ)
#if defined(NXT)
	intrinsic long property(nMotorEncoderTarget, propertyMotorEncoderTargetRelative, kNumbOfTotalMotors, tMotor);
#else
	//intrinsic const long property(nMotorEncoderTargetAbsolute, propertyMotorEncoderTargetAbsolute, kNumbOfTotalMotors, tMotor);
#endif
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	intrinsic void UploadDatalog(compileConst int nStart, compileConst int nSize)
                                      asm(opcdMsgUploadCommands, uploadDatalog, variableCompileConstWord(nSize), variableCompileConstWord(nStart), 0);

	intrinsic const word propertyIndex(nDatalogBufferSize,    propertySystem, kSystemDatalogBufferSize);
	intrinsic const word propertyIndex(nUsedDatalogBytes,     propertySystem, kSystemUsedDatalogBytes);

	intrinsic void AddToDatalog(const int nDataPtIndex, const int nDataValue)  asm(opcdSystemFunctions, byte(sysFuncDatalogAddPoint), variable(nDataPtIndex),  variable(nDataValue));
	//intrinsic void datalogDefineDataPointName(const int nDataPtIndex, const string &sDataName)
	//																													asm(opcdSystemFunctions, byte(sysFuncDataLogDefineDataPointName), variable(nDataPtIndex),  variableRefString(sDataName));
	intrinsic void datalogClear()															asm(opcdSystemFunctions, byte(sysFuncDataLogClear));
	#if defined(bHasDatalogSave)
		intrinsic void SaveNxtDatalog()													asm(opcdSystemFunctions, sysFuncDatalogSaveNext);
	#endif
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	intrinsic word property(nI2CBytesReady,     propertySystem, kSystemNumbI2CBytesReady);
	intrinsic TI2CStatus property(nI2CStatus,   propertySystem, kSystemI2CStatus);

	intrinsic void sendI2CMsg(const byte &sendMsg, const int nReplySize)
									asm(opcdSendI2CMsg, byte(sysFuncSendI2cMsg), variableRefByte(sendMsg), variable(nReplySize));
	intrinsic void readI2CReply(byte &replyBytes, const int nBytesToRead)
									asm(opcdReadI2CMsg, byte(sysFuncReadI2cReply), variableRefByte(replyBytes), variable(nBytesToRead));
